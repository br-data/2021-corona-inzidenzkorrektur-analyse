...
Mit dem im Repo gespeicherten Java-Code wird auf dieses Archiv zugegriffen und die 7-Tage-Fallzahlen für jede Kombination aus Übertragungsdatum, Meldedatum und Landkreis seit dem 1. Januar 2021 aggregiert. Die entsprechende csv.-Datei liegt, ebenso wie eine csv mit den Bevölkerungs- und Geodaten der Landkreise im Ordner input.

## Analyse

#### Einlesen

Die beiden vorbereiteten csv-Dateien werden eingelesen.
...
Dann wird ein Frame mit den Deltas nach 1, 3, 5, und 14 Tagen generiert
...
######### Inzidenzen nach 1/ 3 / 5 /14 Tagen

data_diff <- data_long %>% mutate(Ind = case_when((datenstand == tag_nach_meldezeitraum)  ~ "Datenstand",
                                                  (datenstand-tag_nach_meldezeitraum == 1) ~ "nach_1_Tag",
                                                  (datenstand-tag_nach_meldezeitraum == 3) ~ "nach_3_Tagen",
                                                  (datenstand-tag_nach_meldezeitraum == 5) ~ "nach_5_Tagen",
                                                  (datenstand-tag_nach_meldezeitraum == 14) ~ "nach_14_Tagen")) %>%
                            filter(., !is.na(Ind)) %>%
                            select(., -datenstand) %>%
                            spread(., Ind, Inzidenz ) %>%
                            mutate(., delta_1 = nach_1_Tag-Datenstand, delta_3 = nach_3_Tagen-Datenstand, 
                                      delta_5 = nach_5_Tagen-Datenstand,delta_I4=nach_14_Tagen-Datenstand)
...
Durchschnitte <- data.frame(Zeitraum = c("nach 1 Tag","nach 3 Tagen", "nach 5 Tagen", "nach 14 Tagen"), durschn_veraenderung = c(mean(abs(data_diff$delta_1), na.rm = TRUE),mean(abs(data_diff$delta_3), na.rm = TRUE),mean(abs(data_diff$delta_5), na.rm = TRUE),mean(abs(data_diff$delta_I4), na.rm = TRUE))) 

Durchschnitte %>% DT::datatable(
                rownames = F,
                options = list(
                  dom = "tBp",
                  pageLength = 6,
...
data_diff_rec <- data_diff %>% filter(.,tag_nach_meldezeitraum >= "2021-03-01")

DS_rec <- data.frame(Zeitraum = c("nach 1 Tag","nach 3 Tagen", "nach 5 Tagen", "nach 14 Tagen"), durschn_veraenderung = c(mean(abs(data_diff_rec$delta_1), na.rm = TRUE),mean(abs(data_diff_rec$delta_3), na.rm = TRUE),mean(abs(data_diff_rec$delta_5), na.rm = TRUE),mean(abs(data_diff_rec$delta_I4), na.rm = TRUE))) 
...
data_diff_lk <- data_diff %>% 
  mutate_at(vars(delta_1,delta_3, delta_5,delta_I4), abs) %>%
  group_by(Ags,Rkiname) %>%
  summarise_at(vars(delta_1,delta_3, delta_5,delta_I4), funs(mean, median, max, min), na.rm = TRUE)
...
### Veränderung der Inzidenzwerte pro Tag 

Frage: Um wieviel ändern sich die Inzidenzwerte pro Tag? Nach wie vielen Tagen bleibt der Inzidenzwert nahezu konstant?
