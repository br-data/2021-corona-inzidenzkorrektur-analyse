...
#### Abweichung der 7-Tage-Inzidenzen
...
data_diff_ges <- data_diff %>% mutate(across(c(delta_1, delta_2,delta_3,delta_4,delta_5,delta_I4), abs)) %>%
                                ...
                                mutate(zeitraum = case_when((grepl("1",Art)==TRUE)  ~ "nach 1 Tag",
                                                      ...
                                                      (grepl("_4",Art,fixed =TRUE)==TRUE )  ~ "nach 4 Tagen",
...
data_diff %>% 
  ...
  xlab("Absolute Abweichung der Inzidenz nach 1 Tag") + ylab("Count") +
...
data_diff_bl <- data_diff %>%
                ...
                mutate(across(c(delta_1, delta_2,delta_3,delta_4,delta_5,delta_I4), function (x) (x/Datenstand) * 100)) %>%
                group_by(Bl,State) %>%
                summarise(across(c(Datenstand,delta_1,delta_2,delta_3,delta_4,delta_5,delta_I4), fun[c(1,2,4,5)])) %>%
                mutate_if(is.numeric, function (x) round(x, 1))

data_diff_bl_table <- data_diff_bl %>%
                      relocate(Bl,State,Datenstand_mean,delta_1_mean,delta_2_mean,delta_3_mean,delta_4_mean,delta_5_mean,
                               delta_I4_mean,Datenstand_med, delta_1_med, delta_2_med, delta_3_med, delta_4_med, delta_5_med,
                               delta_I4_med, Datenstand_quart,delta_1_quart,delta_3_quart,delta_5_quart,
                               Datenstand_quant_90,delta_1_quant_90,delta_3_quant_90,delta_5_quant_90) %>%
                      select(1:24) %>%
...
data_diff_bl %>% 
  pivot_longer(names_to= "Tage", values_to = "Mean_Delta", c(delta_1_mean,delta_2_mean,delta_3_mean,delta_4_mean,delta_5_mean,
                               delta_I4_mean)) %>%
...
data_diff_bl %>% 
  pivot_longer(names_to= "Tage", values_to = "Median_Delta", c(delta_1_med, delta_2_med, delta_3_med, delta_4_med, delta_5_med,
                               delta_I4_med)) %>%
...
data_diff_bl %>% 
  pivot_longer(names_to= "Tage", values_to = "Quartile_Delta", c(delta_1_quart, delta_2_quart, delta_3_quart, delta_4_quart, 
                                                                 delta_5_quart,delta_I4_quart)) %>%
...
data_diff_lk %>% 
  ...
  ggtitle("Abweichung der Inzidenzen nach 1,3, und 5 Tagen in den Landkreisen - Median ")
```
...
data_diff_wd %>% 
  ...
  ggtitle("Abweichung der Inzidenzen nach 1, 3 und 5 Tagen je Wochentag")
