...
### Über den Zusammenhang zwischen Höhe des Inzidenzwert und den resultierenden Abweichungen (nach 1 Tag)
...
-> Tabelle zeigt die Abweichungen, wenn zuvor alle Fälle mit einer Startinzidenz > 180 herausgefiltert wurden
...
Das Filtern nach Datenstand > 180 entfernt rund 4600 von rund 27.000 Einträgen (Datenstand: 5. Mai 2021). Die zusammenfassenden Größen sind alle etwas niederer, im Vergleich zur Betrachtung des gesamten Datensatzes. 

Für die folgende Tabelle wurden die 80 Landkreise (20%) herausgenommen, die im betrachteten Zeitraum (seit dem 1. März) die größte Durchschnittsinzidenz (mean) hatten. Dabei fallen lediglich 5200 Einträge heraus  (80*65)  (Datenstand: 5. Mai 2021). Auch hier stellen sich die Werte wieder leicht niederer dar.

-> Indikation dafür, dass die Höhe der Inzidenz mit der Abweichung in einem gewissen Maß zusammenhängt.
...
lk_max <- top_n(ungroup(data_diff_lk),80,Datenstand_mean) %>%
          select(Ags)
lk_max <- as.vector(unlist(lk_max))



data_diff_om <- data_diff %>%  subset(Ags %ni% lk_max) %>%
                                mutate_at(vars(delta_1, delta_2,delta_3,delta_4,delta_5,delta_I4), abs) %>%
                                summarise_at(vars(delta_1, delta_2,delta_3,delta_4, delta_5,delta_I4), 
                                             funs(mean, median, max, quartil=quantile(., probs = 0.75), 
                                                  quantil=quantile(., probs = 0.9)),na.rm = TRUE) %>%
...
data_diff_bl %>% 
  pivot_longer(names_to= "Tage", values_to = "Mean_Delta", 3:8) %>%
...
data_diff_bl %>% 
  pivot_longer(names_to= "Tage", values_to = "Median_Delta", 9:14) %>%
...
data_diff_bl %>% 
  pivot_longer(names_to= "Tage", values_to = "Quantile_Delta", 15:20) %>%
...
- Hängen die Unterschiede zwischen den Bundesländern mit hohen Fallzahlen bzw. hohen Inzidenzen zusammen?

```{r}

index_bl <- tibble(Bl=(c("01","02","03","04","05","06","07","08","09","10","11","12","13","14","15","16")), name=(c("SH", "HH", "NI", "HB", "NW", "HE", "RP", "BW", "BY", "SL", "BE", "BB", "MV", "SN", "ST", "TH"))) 

data_diff_bl_tag <- data_diff %>% mutate_at(vars(delta_1,delta_2,delta_3,delta_4, delta_5,delta_I4), abs) %>%
                group_by(Bl, tag_nach_meldezeitraum) %>%
                summarise_at(vars(Datenstand,delta_1,delta_2,delta_3,delta_4,delta_5,delta_I4), funs(mean, median, quantile(., probs = 0.75), max, sd, mad), na.rm = TRUE) %>%
                select(-c("Datenstand_median","Datenstand_max","Datenstand_sd","Datenstand_mad"))%>%
                left_join(., index_bl, by = "Bl")%>%
                subset(., select=c(41,1:40))%>%
                ungroup()


data_diff_bl_tag %>% 
  pivot_longer(names_to="Art", values_to="Delta_Mean", 5:10) %>%
   filter(Art %in% c("delta_1_mean"))%>%
  ggplot()+
  geom_line(aes(x=tag_nach_meldezeitraum,y=Delta_Mean*10, color=br24_colours[4])) +
  geom_line(aes(x=tag_nach_meldezeitraum,y=Datenstand_mean, color=br24_colours[2]))+
  scale_x_date(labels = scales::date_format("%d.%m"), breaks = "14 days") +
  facet_wrap(.~name)+
  scale_y_continuous(name = "Inzidenz", sec.axis = sec_axis(~./10, name = "Abweichung")) + 
  xlab("Tag_nach_Meldezeitraum") +
  theme(legend.position = "none",panel.grid.major.x = element_line(colour = "grey"),
        axis.text.x = element_text(angle = 90))+
  ggtitle("blau = Mean Inzidenz | orange = Mean Abweichung nach 1 Tag")
...
data_diff_lk %>% 
  pivot_longer(names_to="Art", values_to="Delta_Med", 10:15) %>%
...
Wie sehen die durchschnittlichen Abweichungen im zeitlichen Verlauf aus? Gibt es einen Zusammenhang zwischen höheren Abweichungen und höheren Fallzahlen?
...
data_diff_d <- data_diff %>%
  ...
  summarise(across(c(Datenstand,delta_1,delta_2,delta_3,delta_4, delta_5,delta_I4), fun))

data_diff_d %>% 
  pivot_longer(names_to="Art", values_to="Delta_Med", c(6,12,18)) %>%
...
data_diff_d %>% 
  pivot_longer(names_to="Art", values_to="Median", c(6,12,18)) %>%
  ggplot()+
  geom_line(aes(x=tag_nach_meldezeitraum,y=Datenstand_med, color=(br24_colours[1]))) +
  geom_smooth(aes(x=tag_nach_meldezeitraum,y=Median*10, color= Art))+
  scale_x_date(labels = scales::date_format("%d.%m"), breaks = "4 days") +
  scale_y_continuous(name = "Median Inzidenz", sec.axis = sec_axis(~./10, name = "Median Abweichung")) +
  scale_color_discrete(name = "", labels = c("Median Inz", "delta_1_med","delta_3_med","delta_5_med"))+
  xlab("Tag_nach_Meldezeitraum") +
  theme(panel.grid.major.x = element_line(colour = "grey"),
        axis.text.x = element_text(angle = 90))
...
data_diff_wd %>% 
  pivot_longer(names_to="Tage", values_to="delta_agg", c(2, 4, 6, 8, 10, 12, 14, 16, 18)) %>%
...
data_diff_wd_fl <- left_join(data_diff_wd, data_faelle_wd, by ="weekday") %>%
                    ...
                    relocate(Fallzahl_med,.before = "delta_1_median")
