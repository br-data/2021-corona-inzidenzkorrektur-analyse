...
Aus unseren vorangehenden Recherchen wissen wir, dass ein Grund von Abweichungen hohe Fallzahlen sein können, welche die Gesundheitsämter belasten. Sind die Unterschiede eventuell nur in den hohen Bereichen so drastisch und im Bereich der Grenzwerte nicht ausschlaggebend.
...
Keeps <- data_diff %>% filter(Datenstand <= 180) %>%
          select(c(tag_nach_meldezeitraum,Bl,Rkiname,Ags))
Discards <- data_diff %>% filter(Datenstand > 180) %>%
          select(c(tag_nach_meldezeitraum,Bl,Rkiname,Ags))
...
Discard_Keeps <- Discard_Keeps %>% 
                  ...
                  mutate(Bl = c("SH", "HH", "NI", "HB", "NW", "HE", "RP", "BW", "BY", "SL", "BE", "BB", "MV", "SN", "ST", "TH"),
                         .after = "Nr")%>%
                  mutate("%" = Discards/Keeps*100, .after = "Discards") %>%
...
-> Um die Daten besser vergleichbar zu machen, nehmen wir auch hier die Kombinationen aus Landkreisen und Tagen heraus, bei denen die Inzidenz nach dem "ersten" Datenstand über 180 lag 

```{r}


data_diff_bl <- data_diff %>% filter(Datenstand <= 180) %>%
                mutate(across(c(delta_1,delta_2,delta_3,delta_4, delta_5,delta_I4), abs)) %>%
                group_by(Bl) %>%
...
data_faelle_d <- data_faelle_l %>% 
                 ...
                 set_names("tag_nach_meldezeitraum","Fallzahl")%>%
                 ungroup

data_d_comb <- data_diff_d %>%
  select(c(1,3,6,12,18))%>%
  pivot_longer(names_to="Art", values_to="Median", 3:5) %>%
  left_join(., data_faelle_d, by= "tag_nach_meldezeitraum")

data_d_comb$Fallzahl <- as.integer(data_d_comb$Fallzahl)

data_d_comb %>%
ggplot()+
  geom_col(mapping = aes(x= tag_nach_meldezeitraum, y= Fallzahl/3), fill= br24_colours[11], alpha
           =5/10) +
  geom_line(aes(x=tag_nach_meldezeitraum,y=Datenstand_med, color=br24_colours[1]),size=1.5) +
  geom_smooth(aes(x=tag_nach_meldezeitraum,y=Median*10, color= Art))+
  scale_x_date(labels = scales::date_format("%d.%m"), breaks = "3 days") +
  scale_y_continuous(name = "Inzidenz und Fallzahlen", sec.axis = sec_axis(~./10, name = "Abweichung (Deltas)")) +
