...
Weiter zeigt die Darstellung, dass es auch Fälle gibt, in denen das Delta negativ ausfällt, in denen die Inzidenz also im Nachhinein nach unten korrigiert wurde. Das sind jedoch nur sehr wenige Fälle im meist niedrigen Zahlenbereich. Da aber auch diese Fälle eine Korrektur darstellen und wir in der Auswertung Abweichungen vom ursprünglich vermeldenten Wert untersuchen wollen, gehen sie (vorzeichen-bereinigt) ebenfalls in die Auswertung ein. *Wenn wir im Folgenden von Abweichungen sprechen, können diese (wenn auch nur in sehr wenigen Fällen) auch eine nachträgliche Korrektur nach unten beinhalten.*
...
* Problematisch ist, dass die auftetenden Korrekturen sehr unterschiedlich ausfallen. Im Großteil der Fälle ändern sich die gemeldeten Inzidenzwerte im Nachhinein nur wenig. In einem Viertel der Fälle wird der gemeldete Inzidenzwert  bereits am ersten Tag nach Veröffentlichung um 7 Prozent oder mehr korrigiert, in jedem zehnten Fall sogar um 15 Prozentpunkte oder mehr.  
* In den Folgetage kommt es zu weiteren Anpassungen der Inzidenzwerte. Die Veränderungen zum jeweiligen ursprüngich gemeldeten Wert zeigen, dass der Großteil der für den jeweiligen Zeitraum relevanten Nachmeldungen bereist am ersten Tag nach Veröffentlichung der 7-Tage-Inzidenz beim RKI ankommen.
...
Im Zeitverlauf (aggregiert nach Kalenderwochen) zeigt sich, dass das Ausmaß der nachträglichen Korrekturen in den zurückliegenden Kalenderwochen zurückgegangen ist. Die Meldeketten scheinen nicht mehr so überlastet zu sein wie noch in den Wochen zuvor. Trotzdem wurden auch in den vergangenen beiden Wochen die gemeldeten Inzidenzwerte in einem beträchtlichen Teil der Fälle bereits 1 Tag nach Veröffentlichung deutlich korrigiert.
...
data_diff_kw %>% 
  ggplot() +
    geom_line(aes(x=kw,y=delta_1_quart)) +
    # geom_smooth(aes(x=kw,y=delta_1_quart)) +
    theme(panel.grid.major.x = element_line(colour = "grey")) +
    expand_limits(y = 0) +
    xlab("Kalenderwoche") + ylab("Prozentuale Abweichung") +
    ggtitle("Oberes Quartil der Abweichungen der Inzidenzen nach 1 Tag")
```
