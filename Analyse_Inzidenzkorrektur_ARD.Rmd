---
title: "Auswirkungen des Meldeverzugs auf die 7-Tage-Inzidenz"
...
datenstand = Datenstand beim RKI an, auf den sich die verschiedenen 7-Tage-Inzidenzen beziehen
...
In der Datei matrix_fallzahlen_7tage.csv liegen die 7-Tage-Fallzahlen für jede Kombination aus tag_nach_meldezeitraum und datenstand und Landkreis seit dem 1. Januar 2021 aggregiert. Die entsprechende CSV-Datei liegt, ebenso wie eine CSV-Datei mit den Bevölkerungs- und Geodaten der Landkreise im Ordner input.
...
Die beiden vorbereiteten csv-Dateien werden eingelesen. Der Analysezeitraum wird gewählt. Wir untersuchen die Entwicklung seit dem 1. März 2021 entschieden - dem Beginn der sogenannten dritten Welle.
...
### Abweichungen von der ursprünglich vermeldeten Inzidenz berechnen 
...
Dadurch ergeben sich Abweichungen (deltas), die wir mit der Zahl der vergangenen Tage benennen. delta_1 ist demnach die Differenz zwischen der Ausgangsinzidenz und der Inzidenz für den jweiligen Tag, berechnet mit dem Datenstand 1 Tag später, delta 2 ist die Differenz zwischen der Ausgangsinzidenz und der Inzidenz für den jweiligen Tag, berechnet mit dem Datenstand 2 Tage später, etc.
...
Großteil der vermeldeten Inzidenzen (Landkreis x Tag) liebt im Bereich zwischen 50 und 200, das ist der Kernbereich des Datensatzes.
...
Für die Auswertung rechnen wie die Abweichungen, die wir oben mit den Deltas berechnet haben, in Abweichungen prozentual zur Ausgangsinzidenz um, um sie an die Größenordnung der Ausgangsinzidenz anzupassen und damit besser vergleichbar zu machen. Exemplarisch schauen wir uns die prozentualen Abweichungen nach einem Tag an.

Wie das Histogram zeigt, liegen die meisten Abweichungen nach einem Tag in einer Größenordnung von 1 bis 5 Prozentpunkten (binwidth == 5).  Genauso gibt es aber einen relevanten Teil der Fälle, in denen es zu Korrekturen um 5 oder teilweise deutlich mehr Prozentpunkte kommt. 

Weiter zeigt die Darstellung, dass es auch Fälle gibt, in denen das Delta negativ ausfällt, in denen die Inzidenz also im Nachhinein nach unten korrigiert wurde. Das sind jedoch nur sehr wenige Fälle im meist niedrigen Zahlenbereich. Da aber auch diese Fälle eine Korrektur darstellen und wir in der Auswertung Abweichungen vom ursprünglich vermeldenten Wert untersuchen wollen, gehen sie (vorzeichen-bereinigt) ebenfalls in die Auswertung ein. Wenn ab diesem Punkt also von Abweichungen gesprochen wird, kann diese (wenn auch nur in sehr wenigen Fällen) auch eine nachträgliche Korrektur nach unten sein. 
...
data_diff %>% 
  ...
  xlab("7-Tage-Inzidenzen") + ylab("delta_1") +
  ggtitle("Boxplots der (prozentualen) Abweichungen nach 1 Tag für verschiedene Inzidenzbereiche (binwidth: 10")
...
In der weiteren Analyse verwenden wir die relativen Abweichung in Prozentpunkten vom jeweiligen ursprünglich vermeldeten Inzidenzwert. 

### Deutschlandweit: Prozentuale Abweichungen der 7-Tage-Inzidenzen
...
## Detaillierte Betrachtung der Differenzen

### Nach Bundesländern:

Wir sehen innerhalb der verschiedenen Metriken und Zeiträumen große Unterschiede zwischen den Bundesländern.

Die folgenden Charts zeigen, in welcher Höhe die prozentualen Abweichungen nach x Tagen in den einzelnen Bundesländern liegen - wir stellen einmal das arithmetische Mittel, einmal den Median und einmal das obere Quartil dar.
...
data_diff_bl %>% 
  ...
  theme(legend.position = "none")+
   xlab("Tage nach Veröffentlichung der Inzidenz") + ylab("Prozentuale Abweichung") +
  ggtitle("Abweichung der Inzidenzen in den Bundesländern - Oberes Quartil (75%)")


``` 
Der vollständige, nach Bundesländern aggregierte Datensatz liegt im output-Ordner als CSV-Datei.
...
data_diff_lk %>%
  pivot_longer(names_to="Art", values_to="Delta_Med", c(8, 12, 16)) %>%
...
# data_diff_lk %>%
#   pivot_longer(names_to="Art", values_to="Delta_Med", c(20, 24, 28)) %>%
#   ggplot(aes(x=Ags,y=Delta_Med, group=Art))+
#   geom_point(aes(color=Art), alpha = 0.5) +
#   facet_grid(Art~.)+
#   xlab("Landkreise nach Ags") + ylab("Median_Abweichung") +
